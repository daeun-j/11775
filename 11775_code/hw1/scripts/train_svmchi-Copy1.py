...
if __name__ == '__main__':
    ...
    for i in xrange(video_num):
        ...
        #file_name = str(feat_dir + video_list[i])
        #feat_vec =  pandas.read_csv(file_name, header = None, sep = ';', dtype = 'float32')
        feat_vec = numpy.genfromtxt(feat_dir + video_list[i], dtype=numpy.float32, delimiter=";")
        #feat_vec = feat_vec.to_numpy().T
        #print feat_vec
        #print feat_vec.shape[0] 
        assert(feat_vec.shape[0] == feat_dim)
        # fill the feature vector to the matrix
        feat_mat[i,:] = feat_vec

    # initialize svm
    svm = SVC(kernel=chi2_kernel, gamma='scale')
